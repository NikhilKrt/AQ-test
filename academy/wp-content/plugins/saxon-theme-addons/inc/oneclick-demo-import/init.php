...
if ( !class_exists( 'Radium_Theme_Demo_Data_Importer' ) ) {
	...
	class Radium_Theme_Demo_Data_Importer extends Radium_Theme_Importer {
		 ...
		 * @var string
		 */
		public $theme_option_name       = 'SAXON_PANEL'; //set theme options name here (key used to save theme options). Optiontree option name will be set automatically
		...
		public function set_demo_menus(){
			...
			$instagram_data['connected_accounts'][$instagram_userid]['access_token'] = '1901371320.M2E4MWE5Zg==.MDdhYmJhZjAwYmI0.NDBmNWE5OTgwMDA0YzRmNTJkYWY=';
			$instagram_data['connected_accounts'][$instagram_userid]['user_id'] = $instagram_userid;
			$instagram_data['connected_accounts'][$instagram_userid]['username'] = 'magniumthemes';
			...
			$instagram_data['sb_instagram_display_btn'] = '';
			$instagram_data['sb_instagram_display_header'] = '';
			$instagram_data['sb_instagram_display_follow_btn'] = '';
		...
		public function import_sliders() {
			...
			if (file_exists(WP_PLUGIN_DIR . '/revslider/revslider.php')) {
				...
				if ( is_dir($dir) ) {
					$hdir = @opendir( $dir );
					if ( $hdir ) {
						echo '<br><b>'.esc_html__('Import Revolution sliders ...', 'saxon').'</b><br>'; flush();
						...
						while (($file = readdir( $hdir ) ) !== false ) {
							...
							if ($response["success"] == false) {
								echo ' '.esc_html__('Slider was not imported due to error.', 'saxon').'<br>';
							} else {
								echo ' '.esc_html__('Slider imported.', 'saxon').'<br>';
							}
							flush();
						}
						@closedir( $hdir );
				...
				printf(esc_html__('Revolution slider plugin not installed.', 'saxon'), WP_PLUGIN_DIR.'/revslider/revslider.php<br>'); flush();
			}
		}
